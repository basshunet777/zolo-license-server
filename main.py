...
DB_PATH = os.getenv("DB_PATH", "/data/licenses.db")  # Render disk mount: /data
...
def _ensure_dir_for_db(path: str) -> None:
    """Create parent dir if present. If no dir part (e.g., 'licenses.db'), do nothing."""
...
def resolve_db_path() -> str:
    """
    Prefer DB_PATH (default /data/licenses.db).
    If we can't create/access that dir (e.g., /data without disk), fallback to local ./licenses.db
    so the app still starts.
    """
    preferred = DB_PATH
    try:
        _ensure_dir_for_db(preferred)
        # Try opening to verify permissions
        with db(preferred) as conn:
            conn.execute("SELECT 1")
        return preferred
    except Exception:
        # Fallback: local file in app dir (ephemeral on Render)
        fallback = os.getenv("DB_FALLBACK_PATH", "licenses.db")
        _ensure_dir_for_db(fallback)
        return fallback


EFFECTIVE_DB_PATH = resolve_db_path()


def init_db():
    with db(EFFECTIVE_DB_PATH) as conn:
        conn.execute(
            """
...
def calc_expire(start: datetime, plan: str) -> datetime:
    n, t = parse_plan(plan)
    # Year = 365 days (simple). If you want calendar years, use relativedelta.
    return start + (timedelta(days=n) if t == "D" else timedelta(days=365 * n))


def gen_code(plan: str) -> str:
    parse_plan(plan)  # validate
...
def root():
    return {
        "ok": True,
        "service": "zolo-license-server",
        "db_path": EFFECTIVE_DB_PATH,  # debug uchun: qaysi DB ishlatyapti
    }
...
def list_licenses(x_admin_key: Optional[str] = Header(default=None)):
    ...
    with db(EFFECTIVE_DB_PATH) as conn:
        rows = conn.execute(
            """
            SELECT code, plan, created_at, activated_at, expires_at, revoked, device_id
            FROM licenses
...
def activate(req: ActivateReq):
    ...
    with db(EFFECTIVE_DB_PATH) as conn:
        row = conn.execute(
            """
            SELECT code, plan, activated_at, expires_at, revoked, device_id
            FROM licenses
            WHERE code=?
            """,
            (req.code,),
        ...
        if (
            req.device_id
            and saved_device
            and (req.device_id != saved_device)
            and not ALLOW_MULTI_DEVICE
        ):
